...
private:
    ...
    Nonogram(const Nonogram* const, const uint16_t, const uint16_t); // Constructor for creating a new branch from previous table.
    ...
    static uint16_t margin[2][MAX_NG_SIZE]; // Margin base on size of the table and the clue ([vector][vector index]).
    static bool validity; // Validity of the clue.
    static bool solubility; // Solubility of the nonogram.
    static bool input(std::istream&); // Input the clue from istream and validate clue, return validity.

    NonogramTable table;
    NonogramLine line;
    bool solve(void); // Solve nonogram until the table is no longer updated.
    bool infer(void); // Confirm one line arbitrarily when the table is no longer updated, return solubility.

public:
    static bool solve(std::istream&); // Input the clue from istream.
    static bool isValid(void) noexcept; // Return validity.
    static bool isSolved(void) noexcept; // Return solubility.
    static void print(std::ostream&); // Print table.
};
